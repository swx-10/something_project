...
def cosine_decay_with_warmup(global_step,
   ...
   参数：
    ...
    """
    if total_steps < warmup_steps:raise ValueError('total_steps must be larger or equal to warmup_steps.')
...
batch_size = 5
sample_count = 12608    #样本总数
epochs = 7              # 训练epoch总数
warmup_epoch = 3        # warmup epoch数
learning_rate_base = 0.0001  # warm up后的基础学习率
total_steps = int(epochs * sample_count / batch_size)  # 总的steps
warmup_steps = int(warmup_epoch * sample_count / batch_size)   # warmup 的steps
warmup_batches = warmup_epoch * sample_count / batch_size   # warmup进行的批次
